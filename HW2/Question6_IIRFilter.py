...
A = 0.9
B = 1-A
IIRData.append(data[0])
for i in range(len(data)):
    if i-1 >= 0:
        IIRData.append(A*IIRData[i-1] + B*data[i])

Fs = len(data)/t[-1]  # sample rate
Ts = 1.0/Fs; # sampling interval
ts = np.arange(0,t[-1],Ts) # time vector
y = data # the data to make the fft from
yFiltered = IIRData ##the filtered data for the fft
...
plt.suptitle('Signal D: With A = ' + str(A) + ' and B = ' + str(B))
